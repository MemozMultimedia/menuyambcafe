...
st.markdown("""<style>
    ...
    /* RESET GENERAL Y VISIBILIDAD DE TEXTO */
    html, body, [class*='st-'], .stMarkdown, p, label, span, div {
        font-family: 'Inter', sans-serif !important;
        color: #1e1e1e !important; /* Negro sólido para legibilidad */
    }

    .stApp { background-color: #ffffff !important; }
    [data-testid="stSidebar"] { display: none; }
    
    /* Etiquetas de Inputs y Selectbox */
    ...
    with st.container():
        ...
        if st.button("Ingresar Ahora", use_container_width=True):
            if pin == ROLES_CONFIG.get(rol_sel):
                ...
                st.error("❌ PIN incorrecto para este rol")
